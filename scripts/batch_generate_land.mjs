...
const GENERATE_SCRIPT = path.join(__dirname, 'generate_land.mjs');
...
function getOutputPath(target) {
  ...
  if (target.mode === 'area') {
    return path.join(PROJECT_ROOT, 'data', 'land', 'area', sanitize(target.pref), `${sanitize(target.city)}.json`);
  } else {
    return path.join(PROJECT_ROOT, 'data', 'land', 'station', sanitize(target.rail), sanitize(target.line), `${sanitize(target.station)}.json`);
  }
}
